...
function out_job_live(bool $ok, array $data = [], string $error = ''): never {
    http_response_code(200);
    echo json_encode($ok ? ['ok' => true, 'data' => $data] : ['ok' => false, 'error' => $error], JSON_UNESCAPED_SLASHES);
    exit;
}

function load_job_live_payload(int $jobId): array {
...
try {
    ...
    out_job_live(true, load_job_live_payload($jobId));
} catch (Throwable $e) {
    out_job_live(false, [], $e->getMessage());
}
