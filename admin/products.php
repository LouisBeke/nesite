...
if($_SERVER['REQUEST_METHOD']==='POST'){
 verify_csrf();try{$id=(int)($_POST['id']??0);$action=$_POST['action']??'';
  if($action==='create'){
   ...
   db()->prepare('UPDATE store_products SET stock=?,one_per_customer=? WHERE id=?')->execute([$stock,!empty($_POST['one_per_customer'])?1:0,$id]);
