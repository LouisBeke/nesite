...
$msg = '';
$err = '';
$q = db()->prepare('SELECT s.*,p.name product_name FROM services s LEFT JOIN store_products p ON p.id=s.product_id WHERE s.id=? AND s.user_id=?');
$q->execute([$sid, $u['id']]);
$s = $q->fetch();
if (!$s) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!$pid) throw new RuntimeException('Choose a package.');
        $iid = create_upgrade_invoice($sid, $pid);
        header('Location:/billing.php?upgrade_invoice=' . $iid);
        exit;
    } catch (Throwable $e) {
        $err = $e->getMessage();
    }
...
<head>
    ...
    <title>Upgrade <?= e($s['name']) ?> | FoxNetwork</title>
...
<body class="portal-page"><?php render_client_page_start($u, 'billing', 'Service upgrade'); ?><div class="upgrade-page">
        ...
        <p class="muted">Current package: <b><?= e($s['product_name'] ?? 'Imported / unmapped') ?></b> · €<?= number_format((float)$s['price_monthly'], 2) ?>/month</p><?php if ($err): ?><div class="error"><?= e($err) ?></div><?php endif ?><section class="upgrade-grid"><?php foreach ($products as $p): ?><form method="post" class="card upgrade-card"><input type="hidden" name="csrf" value="<?= e(csrf()) ?>"><input type="hidden" name="service_id" value="<?= e($sid) ?>"><input type="hidden" name="product_id" value="<?= e($p['id']) ?>">
                    ...
                    <ul>
                        <li><?= e($p['ram_mb']) ?> MB RAM</li>
                        <li><?= e($p['cpu_percent']) ?>% CPU</li>
        ...
        <section class="card" style="margin-top:20px">
            <div class="cardhead"><b>CHANGE HISTORY</b></div><?php if (!$hist): ?><div class="empty muted">No package changes yet.</div><?php endif ?><?php foreach ($hist as $h): ?><div class="order-row">
                    <div><b><?= e($h['new_product_name'] ?? 'Package change') ?></b>
                        <div class="muted small"><?= e($h['created_at']) ?></div>
                    </div><span class="order-status status-<?= e($h['status']) ?>"><?= e(strtoupper($h['status'])) ?></span>
                </div><?php endforeach ?>
        </section>
        <p><a class="btn" href="/client">← Back to dashboard</a></p>
