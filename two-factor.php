...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(($_POST['action']??'verify')==='resend'&&$method==='whatsapp'){
        $last=(int)($_SESSION['2fa_message_sent_at']??0);
        if(time()-$last<60)$err='Wait one minute before requesting another code.';
        else{try{messaging_verify_start((string)($u['phone']??''));$_SESSION['2fa_message_sent_at']=time();$err='';}catch(Throwable $e){$err='Could not send a new code. You can still use a recovery code.';}}
    }else{
    ...
    if ($ok) {
        unset($_SESSION['2fa_pending_uid'], $_SESSION['2fa_pending_email'],$_SESSION['2fa_message_sent_at']);
...
<body>
    <div class="auth">
        <form class="authbox" method="post"><img src="/images/logo.png">
            ...
            <p class="muted"><?php $method=(string)($u['two_factor_method']??'totp'); ?>Enter the 6-digit code from <?=e($method==='totp'?'your authenticator app':'the WhatsApp message we sent')?>, or a recovery code.</p><?php if ($err): ?><div class="error"><?= e($err) ?></div><?php endif ?><input type="hidden" name="csrf" value="<?= csrf() ?>">
        ...
        <?php if((string)($u['two_factor_method']??'totp')==='whatsapp'):?><form method="post" style="margin-top:10px"><input type="hidden" name="csrf" value="<?=csrf()?>"><button class="btn wide" name="action" value="resend">Send a new code</button></form><?php endif?>
    </div>
</body>
