<?php
function setting(string $key, $default=null){static $cache=[];if(array_key_exists($key,$cache))return $cache[$key];try{$q=db()->prepare('SELECT setting_value FROM app_settings WHERE setting_key=?');$q->execute([$key]);$v=$q->fetchColumn();return $cache[$key]=$v!==false?$v:$default;}catch(Throwable $e){return $default;}}
function save_setting(string $key,string $value):void{$q=db()->prepare('INSERT INTO app_settings(setting_key,setting_value) VALUES(?,?) ON DUPLICATE KEY UPDATE setting_value=VALUES(setting_value)');$q->execute([$key,$value]);}
...
function smtp_send(string $to,string $subject,string $html):void{
 ...
 smtp_cmd($fp,'EHLO '.($_SERVER['SERVER_NAME']??'my.foxnetwork.be'),[250]);if($sec==='tls'){smtp_cmd($fp,'STARTTLS',[220]);if(!stream_socket_enable_crypto($fp,true,STREAM_CRYPTO_METHOD_TLS_CLIENT))throw new RuntimeException('SMTP TLS negotiation failed.');smtp_cmd($fp,'EHLO '.($_SERVER['SERVER_NAME']??'my.foxnetwork.be'),[250]);}
...
function m365_send(string $to,string $subject,string $html): void {
    $sender=trim((string)setting('m365_sender_email',''));
...
function portal_mail_send(string $to,string $subject,string $html): void {
    if(mail_provider()==='m365') m365_send($to,$subject,$html); else smtp_send($to,$subject,$html);
}
