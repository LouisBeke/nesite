...
function process_mollie_payment(string $paymentId): void {
    ...
    if($status==='paid'){
        ...
        if($oid){$sid=ensure_service_for_order($oid);$sv=service_row($sid);if(empty($sv['ptero_server_id']) && !in_array($sv['status'],['provisioning','active','terminated'],true)){try{provision_service($sid);$sv=service_row($sid);send_template('service_ready',$iv,['service_name'=>$sv['name']]);}catch(Throwable $e){send_template('provision_failed',['id'=>null,'name'=>'Admin','email'=>(string)setting('billing_email',''),'email_notifications'=>1],['service_name'=>$sv['name'],'error'=>$e->getMessage()]);throw $e;}}elseif($sv['status']==='suspended'){unsuspend_service($sid,0);send_template('service_unsuspended',$iv,['service_name'=>$sv['name']]);}}
