...
.listrow .row-actions{display:flex;gap:7px;flex-wrap:wrap;justify-content:flex-end}.manage-form{display:flex;gap:10px;flex-wrap:wrap;align-items:end;padding:16px;border-top:1px solid #252a31;background:#101318}.manage-form label{display:flex;flex:1;min-width:130px;flex-direction:column;gap:6px;color:#9ca4b0;font-size:12px;font-weight:700}.manage-form input,.manage-form select,.manage-form textarea,.manage-input{height:40px;padding:0 10px;background:#0b0e12;color:#fff;border:1px solid #303640;border-radius:8px}.manage-form textarea{height:auto;min-height:80px;padding:10px}.manage-form .checkline{flex:0 0 auto;min-width:auto;flex-direction:row;align-items:center;padding-bottom:10px}.manage-form .checkline input{width:auto;height:auto}.credential-box{margin:14px 16px;padding:14px;border:1px solid #5b4523;background:#2a2115;border-radius:10px;color:#ffd69a}.permission-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;padding:14px 0;max-height:330px;overflow:auto}.permission-grid label{display:flex;gap:8px;align-items:center;padding:8px;background:#101318;border:1px solid #292e36;border-radius:8px;color:#d6dce5;font-size:12px}.permission-grid input{width:auto;height:auto}.task-list{margin:8px 0 0 18px;border-left:2px solid #303640}.task-row{display:flex;justify-content:space-between;gap:10px;padding:9px 12px;color:#cbd2dc;font-size:12px}.empty{padding:18px}
...
.sftp-content{padding:20px}.sftp-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin:18px 0}.sftp-field{display:flex;flex-direction:column;gap:7px}.sftp-field.full{grid-column:1/-1}.sftp-field label{font-size:12px;font-weight:800;color:#9ca4b0;text-transform:uppercase;letter-spacing:.5px}.sftp-copy{display:flex;gap:8px}.sftp-copy input{flex:1;min-width:0;height:42px;padding:0 12px;background:#0b0e12;color:#fff;border:1px solid #303640;border-radius:9px;font:13px Consolas,monospace}.sftp-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px}
...
@media (max-width:640px){.sftp-grid{grid-template-columns:1fr}.sftp-field.full{grid-column:auto}}
@media (max-width:760px){.permission-grid{grid-template-columns:1fr}.listbox .listrow{align-items:flex-start;flex-direction:column}.listrow .row-actions{justify-content:flex-start}}
...
<nav class="nav bottom"><?php if(!empty($_SESSION['admin_return_uid'])):?><a href="/return-admin.php">← Return to Admin</a><?php elseif(($u['role'] ?? '') === 'admin'): ?><a href="/admin/"><span>Admin</span></a><?php endif?><a href="/settings.php">Account Settings</a><a href="/logout.php">Sign out</a></nav>
</aside>
<main class="main">
...
<div class="sftp-grid">
  <div class="sftp-field"><label for="sftphost">Host</label><div class="sftp-copy"><input id="sftphost" readonly value="Loading…"><button class="btn" type="button" data-copy="sftphost">Copy</button></div></div>
  <div class="sftp-field"><label for="sftpport">Port</label><div class="sftp-copy"><input id="sftpport" readonly value=""><button class="btn" type="button" data-copy="sftpport">Copy</button></div></div>
  <div class="sftp-field full"><label for="sftpusername">Username</label><div class="sftp-copy"><input id="sftpusername" readonly value=""><button class="btn" type="button" data-copy="sftpusername">Copy</button></div></div>
  <div class="sftp-field full"><label for="sftpcommand">Command</label><div class="sftp-copy"><input id="sftpcommand" readonly value=""><button class="btn" type="button" data-copy="sftpcommand">Copy</button></div></div>
</div>
...
<section class="pane" id="databases"><div class="manage-card"><div class="cardhead"><b>DATABASES</b></div><div id="dbcredential" class="credential-box" style="display:none"></div><div id="dblist" class="listbox"></div><form class="manage-form" id="databaseform"><label>Database name<input id="dbname" required placeholder="my_database"></label><label>Remote connections<input id="dbremote" value="%" placeholder="%"></label><button class="btn primary">Create database</button></form></div></section>
<section class="pane" id="backups"><div class="manage-card"><div class="cardhead"><b>BACKUPS</b><button id="createbackup" hidden type="button"></button></div><div id="backuplist" class="listbox"></div><form class="manage-form" id="backupform"><label>Backup name<input id="backupname" placeholder="Manual backup"></label><label>Ignored files<input id="backupignored" placeholder="*.log, cache/*"></label><button class="btn primary">Create backup</button></form></div></section>
<section class="pane" id="schedules"><div class="manage-card"><div class="cardhead"><b>SCHEDULES</b></div><div id="schedulelist" class="listbox"></div><form class="manage-form" id="scheduleform"><label>Name<input id="schedulename" required placeholder="Daily restart"></label><label>Minute<input id="scheduleminute" value="0"></label><label>Hour<input id="schedulehour" value="4"></label><label>Day of month<input id="scheduledaymonth" value="*"></label><label>Month<input id="schedulemonth" value="*"></label><label>Day of week<input id="scheduledayweek" value="*"></label><label class="checkline"><input id="scheduleactive" type="checkbox" checked> Active</label><label class="checkline"><input id="scheduleonline" type="checkbox"> Only when online</label><button class="btn primary">Create schedule</button></form></div></section>
<section class="pane" id="network"><div class="manage-card"><div class="cardhead"><b>NETWORK</b><button class="btn primary" id="addalloc">Request allocation</button></div><div id="netlist" class="listbox"></div><div class="muted small" style="padding:14px 16px;border-top:1px solid #252a31">Additional allocations are selected automatically from the node when enabled by the host.</div></div></section>
<section class="pane" id="users"><div class="manage-card"><div class="cardhead"><b>SERVER USERS</b></div><div id="userlist" class="listbox"></div><form class="manage-form" id="subuserform" style="display:block"><label>Email<input id="subuseremail" type="email" required placeholder="user@example.com"></label><div class="muted small" style="margin-top:14px">Permissions</div><div id="permissiongrid" class="permission-grid"></div><button class="btn primary">Invite server user</button></form></div></section>
...
<section class="pane" id="settings"><div class="manage-card"><div class="cardhead"><b>SERVER SETTINGS</b></div><form class="manage-form" id="serveridentityform"><label>Server name<input id="servername" value="<?=e($srv['name']??'Server')?>" required></label><label>Description<input id="serverdescription" value="<?=e($srv['description']??'')?>"></label><button class="btn primary">Save identity</button></form><div class="control-grid"><button class="btn warning" id="settingsreinstall">Reinstall server</button><a class="btn" href="<?=e(rtrim(cfg('pterodactyl.url'),'/').'/server/'.$id)?>" target="_blank" rel="noopener">Open advanced Pterodactyl panel</a></div><div class="error" style="margin:0 16px 16px">Reinstalling can replace files created by the server software. Back up important data first.</div></div></section>
...
function requireSelectedFile(){if(!currentFilePath){alert('Select a file first.');return false}return true}
async function refreshFiles(message=''){
  currentFilePath='';q('#filecontent').value='';q('#filemeta').textContent=message||'Select a file to view or edit.';await loadFiles(currentDir);
}
q('#newfolder').onclick=async()=>{const name=prompt('New folder name');if(!name)return;try{await api('create-folder',{body:{root:currentDir,name}});await loadFiles(currentDir)}catch(e){alert(e.message)}};
q('#newfile').onclick=async()=>{const name=prompt('New file name');if(!name)return;const path=joinPath(currentDir,name);try{await api('save-file',{body:{path,content:''}});await loadFiles(currentDir);await openFile(path)}catch(e){alert(e.message)}};
q('#pullfile').onclick=async()=>{const url=prompt('HTTP or HTTPS URL to download');if(!url)return;const filename=prompt('Save as (leave empty to use the remote filename)')||'';try{await api('pull-file',{body:{url,directory:currentDir,filename}});q('#filemeta').textContent='Remote download started.';setTimeout(()=>loadFiles(currentDir),1500)}catch(e){alert(e.message)}};
...
q('#fileupload').onchange=async event=>{
  ...
  try{
    for(const file of files){
      ...
      const response=await fetch(signed.url+separator+'directory='+encodeURIComponent(currentDir),{method:'POST',body:form});
      if(!response.ok)throw new Error('Upload failed for '+file.name+' (HTTP '+response.status+').');
      done++;
    }
    await loadFiles(currentDir);q('#filemeta').textContent=done+' file(s) uploaded.';
  }catch(e){alert(e.message||String(e))}finally{event.target.value=''}
};
q('#downloadfile').onclick=async()=>{if(!requireSelectedFile())return;try{const d=await api('file-download',{query:{path:currentFilePath}});window.location.href=d.url}catch(e){alert(e.message)}};
q('#renamefile').onclick=async()=>{if(!requireSelectedFile())return;const name=prompt('New file name',basename(currentFilePath));if(!name||name===basename(currentFilePath))return;try{await api('rename-file',{body:{path:currentFilePath,name}});await refreshFiles('File renamed.')}catch(e){alert(e.message)}};
q('#copyfile').onclick=async()=>{if(!requireSelectedFile())return;try{await api('copy-file',{body:{path:currentFilePath}});await loadFiles(currentDir)}catch(e){alert(e.message)}};
q('#compressfile').onclick=async()=>{if(!requireSelectedFile())return;try{await api('compress-file',{body:{path:currentFilePath}});await loadFiles(currentDir)}catch(e){alert(e.message)}};
q('#decompressfile').onclick=async()=>{if(!requireSelectedFile())return;if(!confirm('Extract this archive into the current folder?'))return;try{await api('decompress-file',{body:{path:currentFilePath}});await loadFiles(currentDir)}catch(e){alert(e.message)}};
q('#chmodfile').onclick=async()=>{if(!requireSelectedFile())return;const mode=prompt('Unix permissions (for example 644 or 755)','644');if(!mode)return;try{await api('chmod-file',{body:{path:currentFilePath,mode}});q('#filemeta').textContent='Permissions updated to '+mode+'.'}catch(e){alert(e.message)}};
q('#deletefile').onclick=async()=>{if(!requireSelectedFile())return;if(!confirm('Permanently delete '+basename(currentFilePath)+'?'))return;try{await api('delete-file',{body:{path:currentFilePath}});await refreshFiles('File deleted.')}catch(e){alert(e.message)}};

async function copyValue(id,button){
  const value=q('#'+id)?.value||'';
  if(!value) return;
  try{await navigator.clipboard.writeText(value)}catch(e){const input=q('#'+id);input.focus();input.select();document.execCommand('copy')}
...
async function loadSftp(){
  ...
  }catch(e){q('#sftperror').textContent=e.message||String(e);q('#sftperror').style.display='block';}
}
...
q('#serveridentityform').onsubmit=async event=>{
  ...
  try{
    const name=q('#servername').value.trim(),description=q('#serverdescription').value.trim();
    await api('rename-server',{body:{name,description}});
    document.querySelector('.server-manager h1').textContent=name;
    alert('Server identity updated.');loadActivity();
  }catch(e){alert(e.message)}
};
...
async function loadDatabasesFull(){
  try{
    ...
    q('#dblist').innerHTML=(rows||[]).map(item=>{const a=item.attributes||{};const database=Number(a.id)||0;return `<div class="listrow"><div><b>${em(a.name||'Database')}</b><div class="muted small">${em(a.host?.address||'')}:${em(a.host?.port||'')} · ${em(a.username||'')}</div></div><div class="row-actions"><button class="btn" onclick="rotateDatabase(${database})">Rotate password</button><button class="btn danger" onclick="deleteDatabase(${database})">Delete</button></div></div>`}).join('')||'<div class="empty muted">No databases.</div>';
  }catch(e){q('#dblist').innerHTML='<div class="error">'+em(e.message)+'</div>'}
}
function showDatabaseCredential(data){const password=data?.relationships?.password?.attributes?.password||data?.password||'';const box=q('#dbcredential');if(!password){box.style.display='none';return}box.innerHTML='<b>Save this password now.</b><div style="margin-top:8px;font-family:Consolas,monospace;word-break:break-all">'+em(password)+'</div>';box.style.display='block'}
q('#databaseform').onsubmit=async event=>{event.preventDefault();try{const data=await api('database-create',{body:{name:q('#dbname').value.trim(),remote:q('#dbremote').value.trim()||'%'}});showDatabaseCredential(data);q('#dbname').value='';await loadDatabasesFull();loadActivity()}catch(e){alert(e.message)}};
async function rotateDatabase(database){if(!confirm('Rotate this database password? Existing applications must be updated.'))return;try{showDatabaseCredential(await api('database-rotate',{body:{database}}));loadActivity()}catch(e){alert(e.message)}}
async function deleteDatabase(database){if(!confirm('Permanently delete this database?'))return;try{await api('database-delete',{body:{database}});q('#dbcredential').style.display='none';await loadDatabasesFull();loadActivity()}catch(e){alert(e.message)}}

async function loadBackupsFull(){
  try{
    const rows=await api('backups');
    q('#backuplist').innerHTML=(rows||[]).map(item=>{const a=item.attributes||{};const uuid=encodeURIComponent(String(a.uuid||''));const ready=Boolean(a.is_successful)&&!a.is_uploading;return `<div class="listrow"><div><b>${em(a.name||'Backup')}</b><div class="muted small">${ready?'Ready':(a.is_uploading?'Uploading':'Processing')} · ${a.bytes?Math.round(a.bytes/1048576)+' MB':'—'} · ${em(a.created_at||'')}</div></div><div class="row-actions"><button class="btn" ${ready?'':'disabled'} onclick="downloadBackup('${uuid}')">Download</button><button class="btn" onclick="lockBackup('${uuid}')">${a.is_locked?'Unlock':'Lock'}</button><button class="btn warning" ${ready?'':'disabled'} onclick="restoreBackup('${uuid}')">Restore</button><button class="btn danger" ${a.is_locked?'disabled':''} onclick="deleteBackup('${uuid}')">Delete</button></div></div>`}).join('')||'<div class="empty muted">No backups.</div>';
  }catch(e){q('#backuplist').innerHTML='<div class="error">'+em(e.message)+'</div>'}
}
q('#backupform').onsubmit=async event=>{event.preventDefault();try{await api('create-backup',{body:{name:q('#backupname').value.trim()||'Backup '+new Date().toLocaleString(),ignored:q('#backupignored').value.trim()}});q('#backupname').value='';await loadBackupsFull();loadActivity()}catch(e){alert(e.message)}};
async function downloadBackup(encoded){const backup=decodeURIComponent(encoded);try{const data=await api('backup-download',{query:{backup}});window.location.href=data.url}catch(e){alert(e.message)}}
async function lockBackup(encoded){try{await api('backup-lock',{body:{backup:decodeURIComponent(encoded)}});await loadBackupsFull();loadActivity()}catch(e){alert(e.message)}}
async function restoreBackup(encoded){if(!confirm('Restore this backup? The server will stop during restoration.'))return;const truncate=confirm('Delete all existing files before restoring? Cancel merges the backup with current files.');try{await api('backup-restore',{body:{backup:decodeURIComponent(encoded),truncate}});alert('Backup restore requested.');loadActivity()}catch(e){alert(e.message)}}
async function deleteBackup(encoded){if(!confirm('Permanently delete this backup?'))return;try{await api('backup-delete',{body:{backup:decodeURIComponent(encoded)}});await loadBackupsFull();loadActivity()}catch(e){alert(e.message)}}

async function loadSchedulesFull(){
  try{
    const rows=await api('schedules');
    q('#schedulelist').innerHTML=(rows||[]).map(item=>{const a=item.attributes||{};const schedule=Number(a.id)||0;const tasks=a.relationships?.tasks?.data||item.relationships?.tasks?.data||[];const taskHtml=tasks.length?'<div class="task-list">'+tasks.map(taskItem=>{const task=taskItem.attributes||{};const encoded=encodeURIComponent(JSON.stringify({action:task.action||'command',payload:task.payload||'',time_offset:Number(task.time_offset)||0,sequence_id:Number(task.sequence_id)||1,continue_on_failure:Boolean(task.continue_on_failure)}));return `<div class="task-row"><span>#${em(task.sequence_id||'')} ${em(task.action||'task')}: ${em(task.payload||'')}</span><span><button class="btn" onclick="editTask(${schedule},${Number(task.id)||0},'${encoded}')">Edit</button> <button class="btn danger" onclick="deleteTask(${schedule},${Number(task.id)||0})">Delete</button></span></div>`}).join('')+'</div>':'';const encodedSchedule=encodeURIComponent(JSON.stringify({name:a.name||'Schedule',minute:a.cron?.minute||'*',hour:a.cron?.hour||'*',day_of_month:a.cron?.day_of_month||'*',month:a.cron?.month||'*',day_of_week:a.cron?.day_of_week||'*',is_active:Boolean(a.is_active),only_when_online:Boolean(a.only_when_online)}));return `<div class="listrow"><div><b>${em(a.name||'Schedule')}</b><div class="muted small">${a.is_active?'Active':'Paused'} · ${em(a.cron?.minute||'*')} ${em(a.cron?.hour||'*')} ${em(a.cron?.day_of_month||'*')} ${em(a.cron?.month||'*')} ${em(a.cron?.day_of_week||'*')}${a.only_when_online?' · online only':''}</div>${taskHtml}</div><div class="row-actions"><button class="btn" onclick="runSchedule(${schedule})">Run</button><button class="btn" onclick="editSchedule(${schedule},'${encodedSchedule}')">Edit</button><button class="btn" onclick="addTask(${schedule})">Add task</button><button class="btn danger" onclick="deleteSchedule(${schedule})">Delete</button></div></div>`}).join('')||'<div class="empty muted">No schedules.</div>';
  }catch(e){q('#schedulelist').innerHTML='<div class="error">'+em(e.message)+'</div>'}
}
q('#scheduleform').onsubmit=async event=>{event.preventDefault();try{await api('schedule-create',{body:{name:q('#schedulename').value.trim(),minute:q('#scheduleminute').value.trim(),hour:q('#schedulehour').value.trim(),day_of_month:q('#scheduledaymonth').value.trim(),month:q('#schedulemonth').value.trim(),day_of_week:q('#scheduledayweek').value.trim(),is_active:q('#scheduleactive').checked,only_when_online:q('#scheduleonline').checked}});q('#schedulename').value='';await loadSchedulesFull();loadActivity()}catch(e){alert(e.message)}};
async function runSchedule(schedule){try{await api('schedule-execute',{body:{schedule}});alert('Schedule started.');loadActivity()}catch(e){alert(e.message)}}
async function editSchedule(schedule,encoded){const current=JSON.parse(decodeURIComponent(encoded));const name=prompt('Schedule name',current.name);if(!name)return;const cron=prompt('Cron: minute hour day-of-month month day-of-week',[current.minute,current.hour,current.day_of_month,current.month,current.day_of_week].join(' '));if(!cron)return;const parts=cron.trim().split(/\s+/);if(parts.length!==5){alert('Enter five cron fields.');return}const is_active=confirm('Enable this schedule?');try{await api('schedule-update',{body:{schedule,name,minute:parts[0],hour:parts[1],day_of_month:parts[2],month:parts[3],day_of_week:parts[4],is_active,only_when_online:current.only_when_online}});await loadSchedulesFull();loadActivity()}catch(e){alert(e.message)}}
async function deleteSchedule(schedule){if(!confirm('Delete this schedule and all tasks?'))return;try{await api('schedule-delete',{body:{schedule}});await loadSchedulesFull();loadActivity()}catch(e){alert(e.message)}}
async function addTask(schedule){const task_action=(prompt('Task type: command, power, or backup','command')||'').toLowerCase();if(!task_action)return;const payload=task_action==='backup'?'':prompt(task_action==='power'?'Power action: start, stop, restart, or kill':'Command to run','');if(payload===null)return;const time_offset=Number(prompt('Delay after previous task in seconds (0-900)','0')||0);try{await api('task-create',{body:{schedule,task_action,payload,time_offset,continue_on_failure:false}});await loadSchedulesFull();loadActivity()}catch(e){alert(e.message)}}
async function editTask(schedule,task,encoded){const current=JSON.parse(decodeURIComponent(encoded));const payload=current.action==='backup'?'':prompt('Task payload',current.payload);if(payload===null)return;const time_offset=Number(prompt('Delay in seconds (0-900)',String(current.time_offset))||0);try{await api('task-update',{body:{schedule,task,task_action:current.action,payload,time_offset,sequence_id:current.sequence_id,continue_on_failure:current.continue_on_failure}});await loadSchedulesFull();loadActivity()}catch(e){alert(e.message)}}
async function deleteTask(schedule,task){if(!confirm('Delete this scheduled task?'))return;try{await api('task-delete',{body:{schedule,task}});await loadSchedulesFull();loadActivity()}catch(e){alert(e.message)}}

async function loadNetworkFull(){
  try{
    const rows=await api('network');
    q('#netlist').innerHTML=(rows||[]).map(item=>{const a=item.attributes||{};const allocation=Number(a.id)||0;const notes=encodeURIComponent(String(a.notes||''));return `<div class="listrow"><div><b>${em(a.ip_alias||a.ip)}:${em(a.port)}</b><div class="muted small">${a.is_default?'Primary allocation':'Additional allocation'}${a.notes?' · '+em(a.notes):''}</div></div><div class="row-actions"><button class="btn" onclick="editAllocation(${allocation},'${notes}')">Notes</button>${a.is_default?'<span class="muted small">Primary</span>':`<button class="btn" onclick="setPrimaryAllocationFull(${allocation})">Set primary</button><button class="btn danger" onclick="deleteAllocation(${allocation})">Remove</button>`}</div></div>`}).join('')||'<div class="empty muted">No network allocations.</div>';
  }catch(e){q('#netlist').innerHTML='<div class="error">'+em(e.message)+'</div>'}
}
async function setPrimaryAllocationFull(allocation_id){try{await api('set-primary-allocation',{body:{allocation_id}});await loadNetworkFull();loadActivity()}catch(e){alert(e.message)}}
async function editAllocation(allocation_id,encodedNotes){const notes=prompt('Allocation notes',decodeURIComponent(encodedNotes));if(notes===null)return;try{await api('update-allocation',{body:{allocation_id,notes}});await loadNetworkFull();loadActivity()}catch(e){alert(e.message)}}
async function deleteAllocation(allocation_id){if(!confirm('Remove this additional allocation?'))return;try{await api('delete-allocation',{body:{allocation_id}});await loadNetworkFull();loadActivity()}catch(e){alert(e.message)}}
q('#addalloc').onclick=async()=>{try{await api('add-allocation',{body:{}});await loadNetworkFull();loadActivity()}catch(e){alert(e.message)}};

let availablePermissions=[];
function collectPermissions(source,prefix=''){const out=[];if(Array.isArray(source)){source.forEach(value=>{if(typeof value==='string')out.push({key:value,description:''});else out.push(...collectPermissions(value,prefix))});return out}if(!source||typeof source!=='object')return out;Object.entries(source).forEach(([key,value])=>{if(key==='description')return;const base=prefix?prefix+'.'+key:key;if(typeof value==='string')out.push({key:base,description:value});else if(value&&typeof value==='object'&&value.keys)Object.entries(value.keys).forEach(([child,description])=>out.push({key:base+'.'+child,description:String(description||'')}));else out.push(...collectPermissions(value,base))});return out}
async function loadUsers(){
  try{
    if(!availablePermissions.length){const permissions=await api('permissions');availablePermissions=collectPermissions(permissions);q('#permissiongrid').innerHTML=availablePermissions.map(item=>`<label title="${em(item.description)}"><input type="checkbox" value="${em(item.key)}"> ${em(item.key)}</label>`).join('')||'<div class="muted">No permissions returned.</div>'}
    const rows=await api('subusers');q('#userlist').innerHTML=(rows||[]).map(item=>{const a=item.attributes||{};const uuid=encodeURIComponent(String(a.uuid||''));const permissions=Array.isArray(a.permissions)?a.permissions:[];const encodedPermissions=encodeURIComponent(JSON.stringify(permissions));return `<div class="listrow"><div><b>${em(a.email||a.username||'Server user')}</b><div class="muted small">${permissions.length} permissions${a.two_factor_enabled?' · 2FA enabled':''}</div></div><div class="row-actions"><button class="btn" onclick="editSubuser('${uuid}','${encodedPermissions}')">Permissions</button><button class="btn danger" onclick="deleteSubuser('${uuid}')">Remove</button></div></div>`}).join('')||'<div class="empty muted">No additional server users.</div>';
  }catch(e){q('#userlist').innerHTML='<div class="error">'+em(e.message)+'</div>'}
}
q('#subuserform').onsubmit=async event=>{event.preventDefault();const permissions=Array.from(q('#permissiongrid').querySelectorAll('input:checked')).map(input=>input.value);try{await api('subuser-create',{body:{email:q('#subuseremail').value.trim(),permissions}});q('#subuseremail').value='';q('#permissiongrid').querySelectorAll('input').forEach(input=>input.checked=false);await loadUsers();loadActivity()}catch(e){alert(e.message)}};
async function editSubuser(encodedUser,encodedPermissions){const current=JSON.parse(decodeURIComponent(encodedPermissions));const value=prompt('Comma-separated Pterodactyl permissions',current.join(','));if(value===null)return;const permissions=value.split(',').map(item=>item.trim()).filter(Boolean);try{await api('subuser-update',{body:{subuser:decodeURIComponent(encodedUser),permissions}});await loadUsers();loadActivity()}catch(e){alert(e.message)}}
async function deleteSubuser(encodedUser){if(!confirm('Remove this user from the server?'))return;try{await api('subuser-delete',{body:{subuser:decodeURIComponent(encodedUser)}});await loadUsers();loadActivity()}catch(e){alert(e.message)}}
