...
$rows=db()->query("SELECT i.*,u.name,u.email,u.email_notifications FROM invoices i JOIN users u ON u.id=i.user_id WHERE i.status='unpaid' AND i.due_at<NOW()")->fetchAll();foreach($rows as $i){db()->prepare("UPDATE invoices SET status='overdue',overdue_sent_at=COALESCE(overdue_sent_at,NOW()) WHERE id=?")->execute([$i['id']]);if(empty($i['overdue_sent_at']))send_template('invoice_overdue',$i,['invoice_number'=>$i['invoice_number'],'total'=>number_format((float)$i['total'],2),'currency'=>$i['currency'],'due_date'=>date('d M Y',strtotime($i['due_at']))]);}
...
if(setting('auto_suspend','1')==='1'){
	...
	foreach(db()->query($sql) as $s){
		try{
			if(!empty($s['ptero_server_id']))app_ptero('/servers/'.(int)$s['ptero_server_id'].'/suspend','POST');
...
try{$automationBatch=max(1,(int)setting('automation_batch_size','20'));$aw=automation_run_worker($automationBatch);$out[]='Automation worker: '.$aw['processed'].' processed, '.$aw['failed'].' failed, '.$aw['queued'].' queued.';}catch(Throwable $e){$out[]='Automation worker error: '.$e->getMessage();}
